...
function ComboboxInput({
  ...
  showTrigger = true,
  showClear = false,
  error, // รับค่า error เข้ามา
  ...props
}: ComboboxPrimitive.Input.Props & {
  showTrigger?: boolean
  showClear?: boolean
  error?: boolean | string // เพิ่ม prop error เพื่อใช้ในการแสดงสถานะ error
}) {
  return (
    <InputGroup 
      // ส่ง aria-invalid ให้ InputGroup เผื่อกรณีที่ UI Library จัดการขอบแดงผ่าน Group
      aria-invalid={error ? "true" : undefined}
      className={cn(
        "w-auto", 
        error && "border-destructive ring-destructive/20 focus-within:border-destructive focus-within:ring-3 focus-within:ring-destructive/20",
        className
      )}
    >
