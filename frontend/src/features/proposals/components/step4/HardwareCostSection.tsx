...
export function HardwareCostSection() {
    ...
    <div className="space-y-4">
      ...
      <div className="flex justify-between items-center">
        <h3 className="text-base font-semibold text-foreground">
          ค่าใช้จ่ายครุภัณฑ์คอมพิวเตอร์ที่จัดหาในโครงการ 
        </h3>
        <Button
          type="button"
          variant="default"
        ...
        <div className="divide-y">
          {fields.length === 0 && (
            <div className="p-8 text-center text-sm text-muted-foreground">
              ยังไม่มีรายการครุภัณฑ์ กดปุ่ม "เพิ่มรายการ" เพื่อเริ่มต้น
            </div>
          )}

          {fields.map((field, index) => {
            const refType = watch(`hardwareCosts.${index}.referenceType`);
                ...
                <div className="flex flex-col md:flex-row items-start gap-3">
                  ...
                  <div className="flex-1 w-full">
                    <label className="md:hidden text-xs font-medium mb-1 flex items-center gap-1">
                      <span className="bg-muted px-1.5 rounded-sm text-muted-foreground">{index + 1}</span> ชื่อรายการ
                    </label>
                    <Input
                      {...register(`hardwareCosts.${index}.itemName`)}
                  ...
                  <div className="w-full md:w-24">
                    ...
                    <Input
                      type="number"
                      {...register(`hardwareCosts.${index}.quantity`)}
                  ...
                  <div className="w-full md:w-32">
                    ...
                    <Input
                      type="number"
                      {...register(`hardwareCosts.${index}.unitPrice`)}
                      className="h-9 text-sm text-right"
                    />
                  </div>

                  {/* ที่มาของราคากลาง (ใช้ Controller เพราะเป็น shadcn Select) */}
                  <div className="w-full md:w-48">
                    <label className="md:hidden text-xs font-medium mb-1 block">ที่มาของราคากลาง</label>
                    <Controller
                      control={control}
                      name={`hardwareCosts.${index}.referenceType`}
                      render={({ field }) => (
                  ...
                  <div className="flex-1 bg-muted/40 p-3 rounded-md border border-border/50">
                    ...
                    {refType === "MDES" && (
                      <div className="grid grid-cols-3 gap-3">
                        <div>
                          <Input {...register(`hardwareCosts.${index}.mdesMonth`)} placeholder="ฉบับเดือน  (เช่น มีนาคม)" className="h-8 text-xs bg-background" />
                        </div>
                        <div>
                          <Input {...register(`hardwareCosts.${index}.mdesYear`)} placeholder="พ.ศ. (เช่น 2567)" className="h-8 text-xs bg-background" />
                        </div>
                        <div>
                          <Input {...register(`hardwareCosts.${index}.mdesItemNo`)} placeholder="รายการที่ (เช่น 1)" className="h-8 text-xs bg-background" />
                        </div>
                      </div>
                    )}
                    ...
                    {refType === "MARKET" && (
                      <div className="flex gap-3">
                        <div className="w-32">
                          <Input type="number" {...register(`hardwareCosts.${index}.marketCount`)} placeholder="จำนวน (เช่น 5)" className="h-8 text-xs bg-background" />
                        </div>
                        <div className="flex-1">
                          <Input {...register(`hardwareCosts.${index}.marketCompany`)} placeholder="ใช้ของบริษัท (เช่น บริษัท XYZ)" className="h-8 text-xs bg-background" />
                        </div>
                      </div>
                    )}
                    ...
                    {refType === "PREVIOUS" && (
                      <div className="flex gap-3">
                        <div className="flex-1">
                          <Input {...register(`hardwareCosts.${index}.prevProject`)} placeholder="ชื่อโครงการ (เช่น โครงการ ABC)" className="h-8 text-xs bg-background" />
                        </div>
                        <div className="w-32">
                          <Input {...register(`hardwareCosts.${index}.prevYear`)} placeholder="ปี พ.ศ. (เช่น 2567)" className="h-8 text-xs bg-background" />
                        </div>
                      </div>
                    )}
                    ...
                    {refType === "OTHER" && (
                      <div className="w-full">
                        <Input {...register(`hardwareCosts.${index}.otherDetail`)} placeholder="ระบุเหตุผลและรายละเอียดที่มา" className="h-8 text-xs bg-background" />
                      </div>
                    )}
